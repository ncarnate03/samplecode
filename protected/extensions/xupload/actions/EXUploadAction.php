 ...
 */
class EXUploadAction extends CAction {
    ...
    protected function handleUploading()
    {
        ...
        if ($model->{$this->fileAttribute} !== null) {
        	...
        	$model->{$this->fileNameAttribute} = str_replace(" ", "_", $model->{$this->fileNameAttribute});
...
//                     syslog(LOG_WARNING, $model->{$this->fileNameAttribute});
                    //Yii::app()->getModule('sacms')->updateGallery($model->{$this->fileNameAttribute});
                    Functions::updateGallery($model->{$this->fileNameAttribute});
                } else {
