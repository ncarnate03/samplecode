...
class InvestigationsController extends Controller {
    ...
    public function actionCreate() {
        ...
        if (isset($_POST['Investigation'])) {
            $model->attributes = $_POST['Investigation'];
            if ($model->validate()) {
                if ($model->save()) {
                    $status = 1;
    ...
    public function actionAdmin() {
        $this->pageTitle = "Manage Investigations";
        $model = new Investigation('search');
        $model->unsetAttributes();  // clear any default values
        if (isset($_GET['Investigation']))
            $model->attributes = $_GET['Investigation'];
    ...
    public function loadModel($id) {
        $model = Investigation::model()->findByPk($id);
        if ($model === null)
